...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator animBGColor = ObjectAnimator.ofArgb(target,
                "backgroundColor",
                getBackgroundColor(startColorPicker),
                getBackgroundColor(endColorPicker));
        animBGColor.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        animBGColor.setRepeatCount(ObjectAnimator.INFINITE);
        animBGColor.setRepeatMode(ObjectAnimator.REVERSE);

        // TOD ex2-1：在这里实现另一个 ObjectAnimator，对 target 控件的大小进行缩放，从 1 到 2 循环
        ObjectAnimator animScaleX = getMyAnimator("scaleX", 1, 2);
        ObjectAnimator animScaleY = getMyAnimator("scaleY", 1, 2);

        // TOD ex2-2：在这里实现另一个 ObjectAnimator，对 target 控件的透明度进行修改，从 1 到 0.5f 循环
        ObjectAnimator animAlpha = getMyAnimator("alpha", 1, 0.5f);

        // TOD ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animBGColor, animScaleX, animScaleY, animAlpha);
        animatorSet.start();
    }

    ObjectAnimator getMyAnimator(String propertyName, float valueFrom, float valueTo) {
        ObjectAnimator animResult = ObjectAnimator.ofFloat(target,
                propertyName,
                valueFrom,
                valueTo);
        animResult.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        animResult.setRepeatCount(ObjectAnimator.INFINITE);
        animResult.setRepeatMode(ObjectAnimator.REVERSE);
        return animResult;
    }
}
